...
class VectorRetriever:
    ...
    def search_similar_content(self,
        ...
        try:
            ...
            # 过滤低相似度结果
            results = []
            use_fake_embeddings = isinstance(getattr(self.db_manager, "embeddings", None), FakeEmbeddings)
            for doc, score in search_results:
                metadata = getattr(doc, "metadata", {}) or {}
                is_fallback = metadata.get("_retrieval_mode") == "fallback"
                if is_fallback:
                    threshold = 0.12
                elif use_fake_embeddings:
                    # FakeEmbeddings 分数不稳定，放宽阈值避免“有库却召回为0”
                    threshold = -1.0
                else:
            ...
            # 兜底策略：若过滤后为空但原始结果非空，至少返回一个最相关结果
            if not results and search_results:
                best_doc, best_score = max(search_results, key=lambda x: x[1])
                results = [(best_doc, best_score)]
